---
title: 'PIM inhibitor treatment of PMAr-RS-TAM for 24 hours: initial exploratory analysis and excluding outliers'
...
# get clean meta data
meta <- data.frame(replicate =  c(1, 2, 3, 1, 2, 3, 1, 2, 3),
                   treatment = c(rep("DMSO", 3), rep("MEN1703", 6)),
                   concentration = c(rep("0uM", 3), rep("3uM", 3), rep("5uM", 3)),
                   time = rep("24h", 9))

meta$group <- paste(meta$treatment, meta$concentration, sep = "_")
meta$group[1:3] <- "DMSO"
...
dataPCA$concentration <- vsd$concentration
...
pheatmap(
  ...
  labels_row = colData(vsd)$group,
  labels_col = colData(vsd)$group,
  show_colnames = FALSE,
...
saveRDS(vsd, file = "../data/vsd_MEM_24h.RDS")
saveRDS(dds, file = "../data/dds_MEM_24h.RDS")
```
