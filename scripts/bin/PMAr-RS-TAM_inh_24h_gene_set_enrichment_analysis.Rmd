...
dds <- readRDS("../data/dds_MEM_24h.RDS")
...
for (name in names(fgsea_res[[1]])) {
      write_csv(fgsea_res[[1]][[name]], 
                path = paste0("../data/tables/fgsea/", "fgsea_MEN1703_3uM_24h_", name, ".csv"))
}


for (name in names(fgsea_res[[2]])) {
      write_csv(fgsea_res[[2]][[name]], 
                path = paste0("../data/tables/fgsea/", "fgsea_MEN1703_5uM_24h_", name, ".csv"))
}
...
```{r plotEnrichment_macrophages_5}

plotsEnrich <- list()

for (name in names(Macrophage_signatures)) {
    plotsEnrich[[name]] <- plotEnrichment(Macrophage_signatures[[name]],
               ranks[[2]]) + labs(title=paste0("DMSOvsMEN1703_5uM: ", name))
}
